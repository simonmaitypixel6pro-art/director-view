...
interface BulkExamQRScannerProps {
...
export function BulkExamQRScanner({
  selectedCombinations,
  onAttendanceMarked,
  onClose,
  markedByPersonnelId,
}: BulkExamQRScannerProps) {
  ...
  const processQRCode = async (qrData: string) => {
    ...
    try {
      ...
      console.log("[MYT] Bulk QR - Raw QR data:", qrData)
      console.log("[MYT] Bulk QR - Selected combinations:", selectedCombinations)
      ...
      console.log("[MYT] Bulk QR - Student API response status:", studentRes.status)

      if (!studentRes.ok) {
        const errorText = await studentRes.text()
        console.error("[MYT] Bulk QR - Student API error response:", errorText)
      ...
      console.log("[MYT] Bulk QR - Student data response:", JSON.stringify(studentData, null, 2))
      ...
      const matchingCombination = selectedCombinations.find(
        (combo) => combo.examId === scannedExamId && combo.subjectId === scannedSubjectId,
      )

      if (!matchingCombination) {
        const scanInfo = `Exam ID: ${scannedExamId}, Subject ID: ${scannedSubjectId}`
        const expectedInfo = selectedCombinations.map((c) => `Exam: ${c.examId}, Subject: ${c.subjectId}`).join(" | ")
        if (isMountedRef.current) {
          setMessage({
            type: "error",
            text: `❌ QR not in selected combinations. Scanned: ${scanInfo}. Expected: ${expectedInfo}`,
          })
          setProcessing(false)
        }
        return
      ...
      console.log("[MYT] Bulk QR - Found matching combination, marking attendance...")

      const attendanceRes = await fetch("/api/admin/exams/attendance/mark", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          exam_id: matchingCombination.examId,
          student_id: Number(student.id),
          subject_id: matchingCombination.subjectId,
          course_id: matchingCombination.courseId,
          semester: matchingCombination.semester,
          markedByPersonnelId: markedByPersonnelId || null,
        }),
      })

      const attendanceData = await attendanceRes.json()
      console.log("[MYT] Bulk QR - Attendance mark response:", attendanceData)
      ...
      if (attendanceData.success || attendanceRes.ok) {
        ...
        if (isMountedRef.current) {
          setMessage({
            type: "error",
            text: `❌ Failed: ${attendanceData.message || "Unknown error"}`,
          })
        }
        if (messageTimeoutRef.current) {
          clearTimeout(messageTimeoutRef.current)
      ...
      console.error("[MYT] Bulk QR - Processing error:", err)
      if (isMountedRef.current) {
        setMessage({
          type: "error",
  ...
  useEffect(() => {
    if (!selectedCombinations || selectedCombinations.length === 0) {
      setError("Error: At least one course/semester/exam/subject combination must be selected")
      return
    }
    ...
    const startCamera = async () => {
      try {
        ...
        console.error("[MYT] Bulk QR - Camera error:", err)
        if (isMountedRef.current) {
          const errorMsg =
  ...
  }, [scanning, processing, selectedCombinations])
      ...
      <CardHeader className="bg-muted/50 flex flex-row items-center justify-between">
        <div>
          <CardTitle className="flex items-center">
            ...
            Bulk Hall Tickets QR Scanner
          </CardTitle>
          <CardDescription>Scan QR codes for all selected courses, subjects & semesters at once</CardDescription>
        </div>
        {onClose && (
        ...
        {selectedCombinations.length > 0 && (
          <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
            <p className="text-sm font-medium text-blue-900 dark:text-blue-100 mb-2">
              Active Combinations: {selectedCombinations.length}
            </p>
            <div className="flex flex-wrap gap-2">
              {selectedCombinations.map((combo, idx) => (
                <Badge key={idx} variant="outline" className="text-xs">
                  <Check className="w-3 h-3 mr-1" />
                  Exam {combo.examId} / Subject {combo.subjectId}
                </Badge>
              ))}
            </div>
          </div>
