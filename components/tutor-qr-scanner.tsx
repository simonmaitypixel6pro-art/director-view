...
interface TutorQRScannerProps {
  lectureId: number
  onAttendanceMarked?: (studentId: number, studentName: string) => void
  onClose?: () => void
}

export function TutorQRScanner({ lectureId, onAttendanceMarked, onClose }: TutorQRScannerProps) {
  ...
  const processQRCode = async (qrData: string) => {
    ...
    try {
      ...
      const attendanceRes = await fetch(`/api/tutor/lectures/${lectureId}/qr-attendance`, {
        method: "POST",
      ...
      if (attendanceData.success) {
        ...
        if (isMountedRef.current) {
          setMessage({
            type: "success",
            text: `Marked: ${attendanceData.studentName}`,
          })
          setScannedCount((prev) => prev + 1)
        }
        onAttendanceMarked?.(attendanceData.studentId, attendanceData.studentName)
        ...
        playBeep()
        if (isMountedRef.current) {
          setMessage({ type: "warning", text: attendanceData.message })
        }
        if (messageTimeoutRef.current) {
          clearTimeout(messageTimeoutRef.current)
        }
