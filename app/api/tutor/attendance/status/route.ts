...
export async function GET(request: NextRequest) {
  try {
    ...
    // Rate limiting check
    const rateLimitKey = `tutor_${tutorId}_status`
    ...
    const todayAttendances = await sql`
      ...
      WHERE tutor_id = ${parseInt(tutorId)} AND attendance_date = ${today}
    ...
    console.error("[Tutor Attendance] Error fetching status:", error)
    return NextResponse.json(
