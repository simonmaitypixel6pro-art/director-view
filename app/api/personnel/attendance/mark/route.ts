...
export async function POST(request: NextRequest) {
  try {
    const { personnelId, userType, latitude, longitude, deviceFingerprint } = await request.json()

    // Validate required fields
    if (!personnelId || !userType || latitude === undefined || longitude === undefined || !deviceFingerprint) {
      return NextResponse.json(
        {
          success: false,
          message: "Personnel ID, user type, location coordinates, and device fingerprint are required",
        },
        { status: 400 }
      )
    }
    ...
    // Rate limiting check
    const rateLimitKey = `${userType}_${personnelId}_mark`
    ...
    // Check if device is being used by a DIFFERENT user of the same type today (security check)
    const deviceUsageByOtherUser = await sql`
      SELECT id, user_id, marked_at
      FROM personnel_attendance 
      WHERE device_fingerprint = ${hashedFingerprint} 
        AND attendance_date = ${today}
        AND user_type = ${userType}
        AND user_id != ${personnelId}
      LIMIT 1
    `

    if (deviceUsageByOtherUser.length > 0) {
      return NextResponse.json(
        {
          success: false,
          message: "This device has been used by another user today. One device cannot be used by multiple users on the same day.",
          deviceSharedWithOtherUser: true,
          lastMarkedAt: deviceUsageByOtherUser[0].marked_at,
        },
        { status: 403 }
      )
    }

    // Check if current user already marked with this device today (max 1 per device per day per user)
    ...
    // Check total attendance count for today (max 2 per user per day)
    ...
    // Validate user type to provide better error messages
    const validUserTypes = ['admin_personnel', 'accounts_personnel', 'technical_team', 'peon']
    if (!validUserTypes.includes(userType)) {
      return NextResponse.json(
        {
          success: false,
          message: `Invalid user type: ${userType}. Allowed types are: ${validUserTypes.join(', ')}`,
        },
        { status: 400 }
      )
    }
    ...
    try {
      result = await sql`
        ...
        RETURNING id, user_id, user_type, attendance_date, marked_at, latitude, longitude, attendance_type, campus_location
      `
    } catch (dbError: any) {
      const errorMessage = dbError?.message || String(dbError)
      
      // Check if it's a constraint error
      if (errorMessage.includes('check_valid_user_type') || errorMessage.includes('violates check constraint')) {
        console.error("[Personnel Attendance] Check constraint error - user types mismatch:", {
          userType,
          error: errorMessage,
        ...
        return NextResponse.json(
          {
            success: false,
            message: `User type '${userType}' is not supported for attendance. Please contact the system administrator.`,
    ...
    console.error("[Personnel Attendance] Error marking attendance:", error)
