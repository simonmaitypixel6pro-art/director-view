...
import { ArrowLeft, Camera, Check, Home, AlertCircle, Calendar, Layers, ScanLine } from "lucide-react"
...
export default function ExamAttendancePage() {
  ...
  const loadSubjects = async (examId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        toast({ title: "Error Loading Subjects", description: data.error, variant: "destructive" })
        return
      }
      setSubjects(data)
    } catch (error) {
  ...
  const loadStudents = async (courseId: string, semester: string, examId: string, subjectId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({ title: "Error Loading Students", description: errorData.error, variant: "destructive" })
      }
    } catch (error) {
      toast({ title: "Network Error", description: "Failed to connect", variant: "destructive" })
    }
  ...
  const handleCourseChange = (courseId: string) => { setSelectedCourse(courseId); setSelectedSemester(""); setSelectedExam(""); setSelectedSubject(""); setExams([]); setSubjects([]); setStudents([]) }
  const handleSemesterChange = (semester: string) => { setSelectedSemester(semester); setSelectedExam(""); setSelectedSubject(""); loadExams(selectedCourse, semester) }
  const handleExamChange = (examId: string) => { setSelectedExam(examId); setSelectedSubject(""); loadSubjects(examId) }
  const handleSubjectChange = (subjectId: string) => { setSelectedSubject(subjectId); loadStudents(selectedCourse, selectedSemester, selectedExam, subjectId) }
  const handleQRAttendanceMarked = (studentId: number, studentName: string) => { loadStudents(selectedCourse, selectedSemester, selectedExam, selectedSubject); toast({ title: "Success", description: `Attendance marked for ${studentName}` }) }
        ...
        <motion.div variants={itemVariants} className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-200 dark:border-white/10 pb-6">
          <div className="space-y-1">
            ...
            <p className="text-lg text-muted-foreground dark:text-gray-400">
              Scan student QR codes to mark exam attendance
            </p>
          </div>
        </motion.div>
          ...
          <TabsContent value="single" className="space-y-6 mt-6">
            <motion.div variants={itemVariants}>
                <Card className="bg-white/70 dark:bg-zinc-950/40 border-slate-200 dark:border-white/10 backdrop-blur-md">
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2"><Layers className="w-5 h-5 text-indigo-500" /> Configuration</CardTitle>
                ...
                <motion.div variants={itemVariants} className="space-y-6">
                    ...
                    <div className="flex justify-center md:justify-start">
                        <Button
                            ...
                            className={cn("gap-2 w-full md:w-auto", qrScannerOpen ? "bg-red-600 hover:bg-red-700" : "bg-indigo-600 hover:bg-indigo-700")}
                            size="lg"
                        >
                            ...
                            <div className="flex flex-wrap gap-2">
                                {courses.map(c => (
                                    <Button 
                                        ...
                                        size="sm"
                                        className={cn(bulkSelectedCourses.has(c.id.toString()) && "bg-indigo-600 hover:bg-indigo-700")}
                                    >
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {Array.from({ length: 8 }).map((_, i) => (
                                        <Button
                                            ...
                                            size="sm"
                                            className={cn(bulkSelectedSemesters.has((i+1).toString()) && "bg-indigo-600 hover:bg-indigo-700")}
                                        >
                                            Sem {i+1}
                                        </Button>
                                    ))}
