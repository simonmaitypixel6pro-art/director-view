...
export default function AdminPersonnelAttendancePage() {
  ...
  // Simplified for brevity in this response, assume logic remains the same as provided in prompt
  ...
  // ... (Keep Bulk and Date Logic exactly as provided in prompt)
  const loadBulkExams = async (courseIds: string[], semesters: string[]) => {
    if (courseIds.length === 0 || semesters.length === 0) return
    try {
      const allExams: Array<{ examId: number; examName: string; subjects: Subject[]; courseId: number; semester: number }> = []
    ...
    } catch (error) { console.error("Error loading bulk exams", error) }
  }
  ...
  const handleBulkSubjectToggle = (examId: number, subjectId: number, courseId: number, semester: number) => {
    const combination: SelectedCombination = { courseId, semester, examId, subjectId }
        ...
        <motion.div variants={itemVariants} className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-200 dark:border-white/10 pb-6">
          <div className="space-y-1">
            ...
            <p className="text-lg text-muted-foreground dark:text-gray-400">
              Scan QR codes to mark student attendance for exams
            </p>
          </div>
          ...
          <TabsContent value="single" className="space-y-6 mt-6">
            <motion.div variants={itemVariants}>
                <Card className="bg-white/70 dark:bg-zinc-950/40 border-slate-200 dark:border-white/10 backdrop-blur-md">
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2"><Layers className="w-5 h-5 text-blue-500" /> Configuration</CardTitle>
                ...
                <motion.div variants={itemVariants} className="space-y-6">
                    ...
                    <div className="flex justify-center md:justify-start">
                        <Button
                            ...
                            className={cn("gap-2 w-full md:w-auto", qrScannerOpen ? "bg-red-600 hover:bg-red-700" : "bg-blue-600 hover:bg-blue-700")}
                            size="lg"
                        >
                    ...
                    {qrScannerOpen && (
                        <div className="border rounded-xl overflow-hidden shadow-lg">
                            <ExamQRScanner
                                ...
                                markedByPersonnelId={personnelData?.id}
                            />
                        </div>
                    )}
             ...
             <motion.div variants={itemVariants}>
                <Card className="bg-white/70 dark:bg-zinc-950/40 border-slate-200 dark:border-white/10 backdrop-blur-md">
                    ...
                    <CardContent className="space-y-6">
                        {/* Simplified Bulk Selection UI for brevity - retains logic */}
                            ...
                            <div className="flex flex-wrap gap-2">
                                {courses.map(c => (
                                    <Button 
                                        ...
                                        size="sm"
                                        className={cn(bulkSelectedCourses.has(c.id.toString()) && "bg-blue-600 hover:bg-blue-700")}
                                    >
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {Array.from({ length: 8 }).map((_, i) => (
                                        <Button
                                            ...
                                            size="sm"
                                            className={cn(bulkSelectedSemesters.has((i+1).toString()) && "bg-blue-600 hover:bg-blue-700")}
                                        >
                                            Sem {i+1}
                                        </Button>
                                    ))}
