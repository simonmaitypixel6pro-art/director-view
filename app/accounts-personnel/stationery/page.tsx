...
export default function AccountsPersonnelStationery() {
  ...
  const fetchItems = async () => {
    try {
      const token = localStorage.getItem("accountsPersonnelToken")
      const response = await fetch("/api/accounts-personnel/stationery/items", {
  ...
  const fetchRequests = async () => {
    try {
      const token = localStorage.getItem("accountsPersonnelToken")
      const response = await fetch("/api/accounts-personnel/stationery/requests", {
        headers: { Authorization: `Bearer ${token}` },
      })
      const data = await response.json()
      if (data.success) {
        ...
        <motion.div variants={itemVariants} className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-200 dark:border-white/10 pb-6">
          <div className="space-y-1">
             <div className="flex items-center gap-2 mb-2">
                 <Button 
                    onClick={() => router.push("/accounts-personnel/dashboard")} 
                    variant="ghost" 
                    size="sm" 
